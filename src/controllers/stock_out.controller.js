...
const create_stockOut = asynhandler(async (req, res) => {
  ...
  for (let i = 0; i < itemId.length; i++) {
    ...
    if (record.remainingStock < qty)
      throw new apiError(
        400,
        `Insufficient stock for product: ${item.item_name || productId}`,
      );
...
const update_stockOut = asynhandler(async (req, res) => {
  try {
    ...
    if (!existingStockOut)
      throw new apiError(404, "Stock-Out record not found");
    ...
    for (let i = 0; i < itemId.length; i++) {
      ...
      if (!record)
        throw new apiError(
          400,
          `Opening stock required for product: ${item.item_name || productId}`,
        );

      if (record.remainingStock < difference && difference > 0)
        throw new apiError(
          400,
          `Insufficient stock for product: ${item.item_name || productId}`,
        );
      ...
      const txn = record.transactions.find(
        (t) =>
          t.reference === existingStockOut._id.toString() &&
          t.type === "Stock-Out",
      );
      ...
      if (txn) {
        ...
        txn.Total_sale = Total_sale || 0; // ✅ add Total_sale to history
        txn.date = stockOutDate ? new Date(stockOutDate) : new Date();
      } else {
        // Push new transaction if not exist
        record.transactions.push({
          date: stockOutDate ? new Date(stockOutDate) : new Date(),
          quantity: newQty,
          type: "Stock-Out",
...
const delete_stockOut = asynhandler(async (req, res) => {
  ...
  for (let i = 0; i < existingStockOut.itemId.length; i++) {
    const productId = existingStockOut.itemId[i];
    const qty = existingStockOut.quantity[i];

    // 1️⃣ Add stock back to remainingStock
    await ItemStockRecord.updateOne(
      { productId },
      { $inc: { remainingStock: qty } },
    );

    // 2️⃣ Remove Stock-Out transaction history
    await ItemStockRecord.updateOne(
      { productId },
      {
        $pull: {
  ...
  // 3️⃣ Permanently delete StockOut document
  ...
  res
    ...
    .json(
      new apiResponse(
        200,
        null,
        "Stock-Out deleted & history removed successfully",
      ),
    );
});
...
const dashboard_stockOut = asynhandler(async (req, res) => {
  const stockOutData = await StockOut.find().populate({
    path: "stockOutCategoryId",
    select: "stockoutCategoryName isActive", // jo fields chahiye
  });
  res
    .status(200)
    .json(
